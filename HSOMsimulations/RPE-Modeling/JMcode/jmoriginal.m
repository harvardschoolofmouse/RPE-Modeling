...
L = 3+zeros(1,n); 
L(n-4:end) = .1;      % SD of large uncertainty kernel
web = weber*t;
...
%
%   e=1 --> state uncertainty case, but no sensory feedback
%   e=2 --> state uncertainty case, + sensory feedback at every time
%
for e = 1:2
    for iter = 1:numIter
        for y = 1:T+1
...
% learning without feedback, state uncertainty by Weber law (Weber)
for iter = 1:numIter
    for y = 1:T+1
        Vh(y,3) = w(:,3)'*xw(:,y);
        Vh(y+1,3) = w(:,3)'*xw(:,y+1);
